...
const StatCard = ({ label, value, subtext }) => (
  ...
  >
    <div className="text-xl sm:text-2xl md:text-3xl font-extrabold text-primary mb-2 font-mono break-words">
      {value}
    </div>
    ...
    {subtext && (
      <div className="text-xs text-amber-500 mt-1 font-medium">{subtext}</div>
    )}
  </motion.div>
);
      ...
      <div className="container mx-auto px-4">
        <div className="lg:text-center mb-16">
          ...
          <p className="max-w-2xl mx-auto text-muted-foreground">
            Coding with the same unconventional brilliance and obsession with
            perfection as the modern legend.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <StatCard
            label="Test Average"
            value="56"
            subtext="Best since Bradman"
          />
          <StatCard
            label="Test Centuries"
            value="37"
            subtext="Conversion Master"
          />
          <StatCard label="Style" value="Unorthodox" subtext="Problem Solver" />
          <StatCard
            label="Dedication"
            value="100%"
            subtext="Shadow Batting at 3AM"
          />
        </div>
